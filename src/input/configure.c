									...
									enum e_input__configure_action action)
{
	...
	if (action == INPUT__CONFIGURE_SETUP)
	{
		...
		// will remain the same when the terminal emulator resizes stuff. i.e.
		// if the terminal wraps a row into two, the saved cursor position is
		// wrapped together with the text. This seems to work on all terminals I
		// tried except for the builtin VSCode terminal. But zsh has the same
		// issues as our shell on that terminal so it should be fine.
		//
